...
import { ICON_OPTIONS, CARD_COLORS } from '../constants';
...
const DeckEditorModal: React.FC<DeckEditorModalProps> = ({ deck, onSave, onClose }) => {
    ...
    const SelectedIcon = newCardIconId ? ICON_OPTIONS[newCardIconId] : null;
                            ...
                            {cards.map((card, i) => (
                                <div key={i} className="flex items-center gap-3 bg-slate-100 dark:bg-slate-700 p-2 rounded-md">
                                    <div className={`w-8 h-10 rounded-sm flex items-center justify-center text-white font-bold text-lg ${card.color}`}>
                                        {card.iconId ? <div className="w-5 h-5">{React.createElement(ICON_OPTIONS[card.iconId] || 'div')}</div> : card.value}
                                    </div>
                    ...
                    <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg">
                        ...
                        <div className="flex flex-col md:flex-row gap-6">
                            <div className="flex-grow">
                                <label htmlFor="card-value" className="block text-sm font-bold text-slate-500 dark:text-slate-400 mb-1">Card Value</label>
                                <input
                                    id="card-value"
                                    type="text"
                                    value={newCardValue}
                                ...
                                <div className="mt-4">
                                    <button onClick={() => setShowIconSelector(!showIconSelector)} className="text-sm font-bold text-sky-600 dark:text-sky-400 hover:underline">
                                        {showIconSelector ? 'Hide Icons' : (newCardIconId ? 'Change Icon' : 'Select an Icon (Optional)')}
                                    </button>
                                    {SelectedIcon && !showIconSelector && <SelectedIcon className="inline-block ml-2" />}
                                </div>

                                {showIconSelector && (
                                    <div className="mt-2 p-3 bg-slate-200 dark:bg-slate-700 rounded-md max-h-32 overflow-y-auto">
                                        <div className="grid grid-cols-8 gap-2">
                                             <button onClick={() => { setNewCardIconId(null); setShowIconSelector(false); }} className={`w-full aspect-square flex items-center justify-center rounded-md transition-colors ${!newCardIconId ? 'bg-sky-500 text-white' : 'bg-white dark:bg-slate-600 hover:bg-slate-300 dark:hover:bg-slate-500'}`}>
                                                <X size={18} />
                                            </button>
                                            {Object.entries(ICON_OPTIONS).map(([id, Icon]) => (
                                                <button key={id} onClick={() => { setNewCardIconId(id); setShowIconSelector(false); }} className={`w-full aspect-square flex items-center justify-center rounded-md transition-colors ${newCardIconId === id ? 'bg-sky-500 text-white' : 'bg-white dark:bg-slate-600 hover:bg-slate-300 dark:hover:bg-slate-500'}`}>
                                                    <Icon />
                                                </button>
                                            ))}
                                        </div>
